# v0.5.0 正式版候选验证报告（基于 0.5.0rc3 + RC3-D/E）

- 日期：2026-09-25
- 验证版本：`0.5.0rc3`（含 RC3-D `58c2c5d`、RC3-E `0381f46`）
- 关联 Issue：#6 · 总追踪 #2
- 结论：**除需要 macOS 的 Safari 实机与最终版本号切换外，全部候选门禁在本机通过**。正式发布动作（版本号升 final、tag `v0.5.0`、GitHub Release）按发布计划窗口执行，待仓库所有者确认时机。
...
## 待正式发布清单（版本号切换为 0.5.0 时）

1. `pyproject.toml` / `__init__.py` / PWA / 文档版本号统一升至 `0.5.0`（`check_release_version.py` 守卫）。
